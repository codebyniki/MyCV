...
export const LandingPage = (): JSX.Element => {
  ...
  return (
      <div className="bg-white scroll-smooth overflow-x-hidden">
        <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "backdrop-blur-md bg-white/70 border-b border-gray-200" : "bg-transparent"}`}>
            ...
            <div className="hidden md:flex items-center gap-8 mx-auto">
              {sections.map((item) => (
                  <button
                      ...
                      className={`[font-family:'Montserrat',Helvetica] font-semibold text-base transition-colors ${activeSection === item.id ? "text-[#3827FF]" : "text-[#2a324b]"} hover:text-blue-600`}
                  >
                    {item.label}
                  </button>
              ))}
          ...
          {isMobileMenuOpen && (
              <div className="absolute top-full left-1/2 transform -translate-x-1/2 w-full max-w-[95%] px-4 py-5 z-40 rounded-b-2xl bg-white/80 backdrop-blur-xl shadow-2xl animate-fade-down">
                <div className="flex flex-col space-y-5 text-center">
                  {sections.map((item) => (
                      <button
                          key={item.id}
                          onClick={() => scrollToSection(item.id)}
                          className={`text-base font-semibold transition-all ${activeSection === item.id ? "text-[#3827FF]" : "text-[#2a324b]"} hover:text-[#0077ff]`}
                      >
                        {item.label}
                      </button>
                  ))}
        ...
        <footer className="w-full px-4 md:px-8 lg:px-[94px] pb-[31px] max-w-[1440px] mx-auto">
          ...
          <div className="mt-[30px] flex items-center justify-center md:justify-start">
            <p className="[font-family:'Poppins',Helvetica] font-normal text-black text-sm md:text-xl text-center md:text-left">
              © 2025 – Built with <HeartIcon className="inline w-4 h-4 md:w-6 md:h-6 text-[#3827FF] mr-1" /> by Nikola Stanic
            </p>
          </div>
        </footer>
